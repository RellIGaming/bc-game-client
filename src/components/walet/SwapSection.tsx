...
const SwapSection = () => {
  const { balances, fetchSwapBalances, doSwap, swapRate, fetchRate, } = useWalletStore();
  ...
  useEffect(() => {
    ...
    setToAmount((Number(fromAmount) * swapRate).toFixed(2));
  }, [fromAmount, swapRate]);

  // ✅ get balance
  const fromBalance =
    balances.find((b) => b.currency === fromCurrency)?.balance || 0;

  // ✅ percentage click
  const handlePercent = (percent: number) => {
    const value = (fromBalance * percent) / 100;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-sm text-gray-400">  ≈ {toCurrency} {toAmount || "0.00"}</span>
      </div>
      ...
      <div className="flex justify-center my-5">
        <button onClick={handleSwap} className="bg-secondary p-3 rounded-lg hover:bg-[#30363a]">
      ...
      <div className="flex justify-between text-sm text-gray-400 mt-4">
        <span>Deposit Balance: {fromBalance}</span>
      ...
      <div className="mt-6 space-y-3 text-sm">
        ...
        <div className="flex justify-between text-gray-400">
          ...
          <span className="text-primary">
            1 {fromCurrency} ≈ {swapRate || 0} {toCurrency}
          </span>
        </div>
      ...
      <button
        onClick={async () => {
          ...
          setFromAmount("");
          setToAmount("");
        }} className="w-full bg-primary hover:bg-green-600 mt-2 py-2 rounded-lg font-semibold">
        Swap Now
      </button>
    </div>
